...
当TCP传输运行在局域网中是相当舒服的，但是如果是需要接入互联网，那么就会遇到类似路由缓存减慢包的传输等问题，发送端就需要方法去降低自己的发送速度。慢启动算法要求发送方维护一个拥塞窗口，当和某个主机连接是窗口初始化为1，然后没收到一个ack则将窗口加一个报文段的长度，因此如果连续能收到ack的话，则增长的速度是成倍的，直到出现拥塞。那么对于发送方而言，它只需要发送拥塞窗口和接收窗口的最小值就可以了。**要注意的是，拥塞窗口时由发送方维护，但是接收窗口是由接受方维护。**这个结合后面的重传机制一起详细来讲。
...
之前谈到了慢启动，那是在刚建立连接时开始初始化整个连接。那么当连接遇到了拥塞的情况，要如何应对呢？

事实上一个主机是无法直接告知另一端是丢包，拥塞还是怎么了，如果出现了丢包或者拥塞，那么可能会出现两种情况：不停收到冗余ack和传输超时。后者依赖于RTT的计算。

先看一下拥塞避免算法是怎么完成的：之前提到当出现拥塞状况时，由发送端会维护一个拥塞窗口，限制报文的发送速率。同时，主机还会维护一个阈值ssthresh，这个阈值是慢启动阈值。

1. 首先初始化拥塞窗口为一个报文长度（字节表示），并初始化ssthresh为65535字节。
2. 发送方发送报文的长度一定是被限制在cwnd和接收窗口最小值的范围内的。
3. 当拥塞发生时，对两种不同的情况会有不同的应对。如果主机检测到发送超时，则将cwnd设置为一个报文长度，当前的慢启动阈值变为当前窗口大小的一半，然后执行慢启动算法；如果是主机接收到了三个冗余的ack，则当前的慢启动阈值变为当前窗口大小的一半。
4. 当cwnd小于ssthresh时，系统进行慢启动；否则进入拥塞避免，慢启动会直到拥塞发生时报文段大小的一半处；而拥塞避免保证cwnd的值会随着每一次ack的获取增加1/cwnd。

**快速重传和快速恢复算法的实现**：

1. 当三个冗余ack被获取到后，发送者需要设置ssthresh的值（上面步骤中有提到），然后重传报文，然后将cwnd的值设置为ssthresh加上三个报文段的长度。
2. 每一次收到冗余的ack后增加cwnd的值，并且继续发送报文。
3. 当下一个ack获得后，将cwnd设置为ssthresh的值。

这里可以解释一下为什么收到三个冗余ack之后不需要和超时一样重新进入初始慢启动状态，那是因为如果收到冗余ack则说明传输的拥塞程度可能并没有那么大，接收端依旧有能力在一段时间内返回ack。

#### 报文段的重组

其实这个技术早在之前将delay ack的时候就已经有所接触了，这里讲一下如果存在下面这种情况。如果在传输过程中主机端发送了两个不同的连续报文后，都没有收到ack，当超时后将重新发送。此时发送方就没必要再次发送两个不同的报文，而是可以将两次发送的内容放在同一个报文中，只要其中的内容长度超过MSS。这就是TCP做到的报文段的重组。

## TCP 坚持计时器

我真的不知道怎么翻译这个，名字叫做Persist Timer。

想象这样一个问题，此时接收方的窗口时0，他在下一步等待其有空间的情况下需要告知发送方可以发送数据了，但是这个ack却丢失了，那么此时发送方和接受方就进入了一个死锁的情况中。这时需要发送者有一个机制去试探接收者窗口是否打开，这就需要所谓**窗口试探**以及坚持计时器的帮助。

当发送者收到了接收者提示接收窗口为0的报文后，发送者开启了它的坚持计时器。当这个计时器超时时，它会去探测窗口是否为空。计时器时长的计算使用了指数级别增长的办法。这个探测包和重传机制最大的不同是它不会放弃发送探测包，会一直发下去，直到打开接收窗口。

#### ？傻子窗口综合征，可以了解一下

## TCP 保活计时器

当客户端和服务器端什么事情都不干的时候，它们是怎么知道这个连接还是存在的呢？由服务器实验一个叫做保活计时器，要去探测客户端是否还存活。当服务器的保活计时器超时，发送一个探测包来探测客户端，那么一共可能有以下四种情况：

1. 客户端存活并回应，这个时候服务器重新设置保活计时器，如果其中在超时过程中有任何通信，则重新设置计时器。
2. 如果客户端不回应，则服务器会发送10次探测包，如果都没有回应则认为客户机已经关闭，断开连接。
3. 如果服务器端收到的是一个reset报文段，服务器会中终止连接。
4. 如果客户机是存活的，但是这个探测数据包没有到达客户端，那么和情况2一样。
